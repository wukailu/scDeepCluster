...
def encoder(h, dims, noise_sd=0.0, init='glorot_uniform', act='relu', dense=True, source=None):
    ...
    for i in range(len(dims) - 2):
        ...
        bn = source.get_layer(name=f'en_bn_{i}') if source else BatchNormalization(name=f'bn_{i}')
        h = bn(h)
        h = Activation(act)(h)
        if dense:
...
def decoder(h, dims, noise_sd=0.0, init='glorot_uniform', act='relu', dense=True, source=None):
    ...
    for i in range(len(dims) - 2, 0, -1):
        fc = source.get_layer(name=f'decoder_{i}') if source else Dense(dims[i], kernel_initializer=init,
                                                                        name=f'decoder_{i}')
        h = fc(his)
        bn = source.get_layer(name=f'de_bn_{i}') if source else BatchNormalization(name=f'bn_{i}')
        h = bn(h)
        h = Activation(act)(h)
        if dense:
            his = Concatenate(axis=1)([his, h])
...
if __name__ == "__main__":
    ...
    scDeepCluster = SCDeepCluster(dims=[input_size, 256, 64, 32], n_clusters=args.n_clusters,
